...
export default function KirkensHistorie() {
  ...
  return (
    <Layout>
      <GradientDivider className="h-1.5 from-gold via-navy to-gold" />

      <section className="relative bg-gradient-to-b from-navy via-navy to-[#2d4a9a] text-white overflow-hidden">
      ...
      <div className="bg-cream w-full">
        <div className="max-w-5xl mx-auto px-6 py-20">
          <div className="mb-20 text-center max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-full border-2 border-gold mb-6">
              <MapPin className="text-gold" size={18} />
              <span className="text-sm font-bold text-navy">Odense, Danmark</span>
            </div>
            <h2 className="font-serif text-4xl md:text-5xl font-bold text-navy mb-6">En kirke med rødder og vinger</h2>
          ...
          <div className="relative">
            <div className="absolute left-8 md:left-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-gold via-navy to-gold" />
                ...
                return (
                  <div key={index} className="relative">
                    <div className={`${isLeft ? 'md:pr-[calc(50%+3rem)]' : 'md:pl-[calc(50%+3rem)]'}`}>
                      <div className={`bg-white rounded-2xl border-2 ${event.highlight ? 'border-gold shadow-xl' : 'border-slate-200 shadow-lg'} p-8 relative ml-16 md:ml-0 hover:shadow-2xl transition-shadow duration-300`}>
                        <div className={`absolute left-0 md:left-auto ${isLeft ? 'md:right-0' : 'md:left-0'} top-8 w-16 h-16 ${event.highlight ? 'bg-gradient-to-br from-gold to-[#d4b06a]' : 'bg-gradient-to-br from-navy to-[#2d4a9a]'} rounded-xl flex items-center justify-center shadow-lg -ml-8 md:ml-0 ${isLeft ? 'md:-mr-8' : 'md:-ml-8'}`}>
                          <Icon className="text-white" size={28} />
                        </div>
                        <div className={isLeft ? '' : 'md:text-right'}>
                          <div className="inline-block px-4 py-1.5 bg-navy rounded-full mb-4">
                            <span className="text-white font-bold text-lg">{event.year}</span>
                          </div>
                          <h3 className="font-serif text-3xl font-bold text-navy mb-4">{event.title}</h3>
                          <p className="text-slate-700 leading-relaxed text-lg">{event.description}</p>
                        </div>
                      </div>
                    </div>
                    <div className="absolute left-8 md:left-1/2 top-8 w-3 h-3 bg-gold rounded-full border-4 border-cream -ml-[5px] md:-ml-[5px] z-10" />
                  </div>
                );
              })}
            </div>
          ...
          <div className="mt-24 grid md:grid-cols-2 gap-8 relative z-10 bg-cream">
            <div className="bg-white rounded-2xl border-2 border-slate-200 p-8 md:p-10 shadow-lg relative overflow-hidden group hover:border-gold transition-colors duration-300">
              <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-navy/5 to-transparent rounded-bl-full" />
              <div className="relative">
                <div className="w-14 h-14 bg-gradient-to-br from-navy to-[#2d4a9a] rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
                  <Church className="text-white" size={28} />
                </div>
                <h3 className="font-serif text-3xl font-bold text-navy mb-4">Arkitektur</h3>
            ...
            <div className="bg-gradient-to-br from-navy to-[#2d4a9a] rounded-2xl p-8 md:p-10 text-white shadow-xl relative overflow-hidden">
              <div className="absolute inset-0 opacity-10">
                <div className="absolute inset-0" style={{ backgroundImage: `radial-gradient(circle at 2px 2px, white 1px, transparent 0)`, backgroundSize: '24px 24px' }} />
              </div>
              <div className="relative">
                <div className="w-14 h-14 bg-gold rounded-full flex items-center justify-center mb-6">
                ...
                <div className="space-y-3">
                  {interiorElements.map((element, index) => (
                    <div key={index} className="flex items-start gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-gold">{element.title}</div>
          ...
          <div className="mt-16 bg-white rounded-2xl border-2 border-slate-200 p-8 md:p-12 relative overflow-hidden">
            <div className="absolute bottom-0 right-0 w-48 h-48 bg-gradient-to-tl from-gold/10 to-transparent rounded-tl-full" />
            <div className="relative">
              <div className="max-w-3xl mx-auto text-center">
                <div className="inline-block p-4 bg-cream rounded-full mb-6">
                  <Sparkles className="text-gold" size={32} />
                </div>
                <h3 className="font-serif text-3xl md:text-4xl font-bold text-navy mb-6">En levende tradition</h3>
                ...
                <div className="inline-block px-6 py-2 bg-navy text-white rounded-full">
